...
```{r pressure, echo=FALSE, fig.cap="\\label{fig:pressure} Pressure"}
plot(pressure)
```
...
```{r, echo = FALSE, fig.cap= "Dataset and regression", Regression}
...
The \acrshort{itt} analysis ensures that all randomized participants are included, regardless of whether they adhered to the treatment protocol. We employed \acrshort{iv} estimation to address potential endogeneity in the treatment assignment. The \acrshort{sbcf} model was applied to estimate heterogeneous treatment effects across subpopulations. To account for unobserved confounding, we extended our analysis using the \acrshort{sbcf-iv} approach.
